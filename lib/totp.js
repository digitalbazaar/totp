...
export async function generateSecret({
  algorithm = 'SHA-1', digits = 6, period = 30
} = {}) {
...
export async function generateToken({
  ...
  return _generateToken({steps, algorithm, secret, digits});
}
...
export async function verify({
  ...
  return candidates.some(c => c === verifyValue);
}

// https://github.com/google/google-authenticator/wiki/Key-Uri-Format
export function toKeyUri({
  accountname, secret, algorithm, period, digits, issuer
} = {}) {
  if(issuer.includes(':')) {
...
function _generateVerifyValue({steps, algorithm, secret, digits}) {
  const token = await _generateToken({steps, algorithm, secret, digits});
  return hmac.hmac({algorithm, secret, data: token});
}

function _generateToken({steps, algorithm, secret, digits}) {
  // calculate the number of steps since time=0
