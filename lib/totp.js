 ...
 */
export async function generateSecret({algorithm = 'SHA-1'} = {}) {
  const size = hmac.MIN_SECRET_SIZE.get(algorithm);
  if(size === undefined) {
    throw new Error(`Unsupported hash algorithm "${algorithm}".`);
  }
  const secret = await hmac.generateSecret({size});
...
export async function verify({
  ...
  for(let i = -delta; i <= delta; ++i) {
    const steps = Math.max(0, currentSteps + i);
...
export function toKeyUri({
  ...
  if(algorithm !== 'SHA-1') {
    params.algorithm = algorithm.replace(/-/g, '');
  }
  if(period !== 30) {
...
export function fromKeyUri({uri} = {}) {
  ...
  // `host` and `pathname` may be parsed differently on different platforms;
  // normalize them
  let {host, pathname} = parsed;
  if(host === '') {
    // in this case see if `pathname` is parsed as `//totp/...`
    if(pathname.startsWith('//')) {
      const nextSlash = pathname.indexOf('/', 2);
      host = pathname.substring(2, nextSlash === -1 ? undefined : nextSlash);
      pathname = pathname.slice(2 + host.length);
    }
  }
  if(host !== 'totp') {
...
export function base32Decode(string) {
  const length = Math.floor(string.length * 5 / 8);
  return new Uint8Array(_base32Decode(string, BASE32_VARIANT), 0, length);
}
